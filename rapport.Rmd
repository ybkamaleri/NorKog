...
# Demografiskef variabler, gjennomsnitt per poliklinikk og missing
...
## variable HOVEDDIAGNOSE brukes med allered byttet til "diag" fra forrige analysen
...
## gir senter navn
t.diag <- senter[n.diag] #right outer join

t.diag <- diagTabell[t.diag, on = c(kode = "diag")]

## konvertere til wide
s.diag <- dcast(t.diag, Navn ~ diagnose, value.var = "prop")

## bytt missing med 0
bNA(s.diag)

## tar bort kollone som ikke trenges
del.col <- "Annen diagnose"
s.diag <- s.diag[, (del.col) := NULL]

## reorganisere kollone
diag.colorder <- c("Navn", 
                   "AD",
                   "SCI",
                   "MCI",
                   "VaD",
...
setnames(tabell.md, c("Navn", 1000), c("Poliklinikker", yrtab))
setkey(tabell.md, Poliklinikker, diagnose)

library(pander)
panderOptions("digits", 1)
pander(tabell.md)

## kable(tabell.md, format = "latex", booktabs = T, caption = "Longtable") %>%
