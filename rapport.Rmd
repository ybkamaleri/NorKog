...
Data <- Data[, date_yr := as.IDate(DATO_US, format = "%m/%d/%Y")] #konvertere DATO_US til date.format
Data[, yr := as.integer(format(date_yr, "%Y"))]
dateNA <- Data[is.na(yr), .N] # Antall DATO_US == NA

## Dataset uten NA i DATO_US
Datayr <- Data[, .SD[!is.na(yr)]] #exclude DATO_US == NA
yrmix <- 2008:2012 #slå sammen disse år tallene
yrtab <- "2008-2012" #tekst til tabell
Datayr[, yr2 := ifelse(yr %in% yrmix, 2000, yr)] #yr2000 = 2008:2012

```

Det er `r dateNA` sentre mangle **DATO_US**. Derfor antallet er ikke lik som i forrige figur
...
names(tab02all)[names(tab02all) == 2000] <- yrtab
