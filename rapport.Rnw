...
%% ===========================%%
%% Valg dynamiske variablene %%
%%===========================%%
<<var-valg, include=FALSE, echo=FALSE, cache=FALSE>>=
...
## Hvilket år skal kombineres/grupperes? - endres etter behov
## dato fra 2008 til 2013 skrives 2008:2013
yearMix <- 2008:20114

## Tekst posisjon inne eller utenfor bar i figur 1
## hvor deling skal gjøres? Alle tellene under dette tallet skal ligger utenfor søylen.
deltxt <- 80

## Språk
## Hvis senternavn ikke vises riktig bukstaver, valg mellom "UTF-8" og "Latin-1"
spraak <- "Latin-1"

############################################
## Alt under denne linjen bør ikke endres ##
## om du ikke er kjent med R og LaTeX     ##
############################################
...
\title{Kvartalsrapport for Norkog\vspace{-1.5cm}}
\date{}
...
%% ved bruk \maketitle lages en blank første side
% \begin{center}
% \textbf{\maketitle}
% \par\large\textbf{\Sexpr{format(Sys.Date(), "%d %b %Y")}}
...
\begingroup
 \centering
 \huge\textbf{Kvartalsrapport for Norkog}
...
pakker <- c("data.table", "ggplot2", "directlabels", "grid", "knitr", "kableExtra", "haven")
load(pakker)
...
############################
## Global chunk oppsett
###########################
##lager mappe for å lage figurer
knitr::opts_chunk$set(fig.path = 'figures/nkog-', echo = FALSE, message = FALSE, warning = FALSE)
options(digits = 3)
...
kilde <- "~/avid/norkog/datakilder1.R"
source(kilde)
...
senter <- data.table::fread(sentfil, header = TRUE, encoding = spraak)
...
\section{Antall inkluderte per senter}
...
Det er \Sexpr{inkl} sentre inkludert i analysen.
\newline
<<antall-output, fig.align='center'>>=
## Legge til gjennomsnitt hele landet
DTsnitt <- data.table(Navn = "Gjennomsnitt", N = round(NN / sn, digits = 0))
...
\section{Antall inkluderte per senter per år}
...
Antall pasienter kan være ulik med forrige figur om det er missing data i
innregistreringen av \texttt{US\_AARSTALL}. I denne analysen er det \textbf{\Sexpr{yrNA}} missing data i
\texttt{US\_AARSTALL}.

<<inkludert-outp, results='asis'>>=
...
\section{Demografiske variabler, gjennomsnitt per senter og missing}
...
herfra. Total pasienter som brukes i analysen er \textbf{\Sexpr{dataN}}

<<demografisk-output, results='asis'>>=
...
\section{Motakker av offentlig hjelp}

<<mottak-setup, include=FALSE>>=
...
ggplot(offPro, aes(as.factor(yr), pros, group = get)) +
...
title <- "Andel og antall for diagnosekategorier i NorKog"
...
\section{Diagnoser per senter}

<<diagsent, results='asis'>>=
...
\section{Demens og MCI, per senter per år}
...
n.demens[, prop := round(N / sum(N)*100, digits = 1), by = list(Senter, yr2)]
...
n.mci[, prop := round(N / sum(N)*100, digits = 1), by = list(Senter, yr2)]
