...
```{r fig02, echo=FALSE, warning=FALSE, message=FALSE, fig.align='center'}
##labelling
fig_lab <- c("Nei", "Ja", "Missing")
...
gg.fig02 <- ggplot(offPro, aes(as.factor(yr), pros, group = get, color = as.factor(get))) +
  geom_line(size = 1) +
  geom_point(stat = "identity") +
  scale_x_discrete(expand=c(0, 1.5)) +
  geom_text(aes(label = format(pros, digits = 1)), vjust = -1.2, color = "grey40", size = 3) +
  ## dl.trans for å posisjonere hvor teksten skal vises
  directlabels::geom_dl(aes(label = get), method = list(dl.trans(x = x + 0.3), "last.points")) + 
  theme_bw() +
