...
fig01Data <- rbindlist(list(fig01Data, DTsnitt)) #kombinere gjennomsnitt til datasett

## Bestemmer posisjonere for tekst - < 100 utenfor søyle
...
gg.fig1 <- ggplot(innText, aes(reorder(Navn, N), N)) +
  geom_bar(stat = "identity", aes(fill = Navn == 'Gjennomsnitt')) +
...
Data[, yr2 := ifelse(get(yr) %in% yrmix, 2000, get(yr))] #dummy for 2008:2012 er 2000

## Dataset uten NA i DATO_US
Datayr <- Data[, .SD[!is.na(get(yr))]] #exclude DATO_US == NA

```

Det er `r yrNA` missing data i **DATO_US**. Derfor antallet er ikke lik som i forrige figur
...
vb03 <- grep("^KJ.*NN$", colnames(DataT02))
...
vb02 <- grep("_SKOLE$", colnames(DataT02))#finne index for ÅR_SKOLE
